        ...
        <transition name="el-zoom-in-center">
          ...
          >
            <el-row type="flex" align="middle" class="nav-title" v-if="DeptAuth">
              <el-button @click="openGroupForm('add')" type="success">添加部门</el-button>
            </el-row>
        ...
        <el-main>
          <div class="t-header">
            <el-row type="flex" align="middle">
              <el-col :span="12" >
                <el-button @click="openGroupForm('update')" type="primary" v-if="DeptAuth" :disabled="showList">修改部门信息</el-button>
...
export default {
  ...
  methods: {
    ...
    //http获取“用户组”列表
    getDeptList() { 
      this.$store.dispatch("admin/get_DeptList", {communityuuid:this.communityuuid});
    },
    async changeMember() {
    ...
    appendGroup() {
      ...
      this.$refs["GroupForm"].validate(async valid => {
        if (valid) {
          ...
          await addDept({...this.GroupForm,communityuuid:this.communityuuid})
            .then(({ data }) => {
    ...
    updateGroup() {
      ...
      this.$refs["GroupForm"].validate(async valid => {
        if (valid) {
          ...
          await putDept({...this.GroupForm, communityuuid:this.communityuuid})
            .then(({ data }) => {
              this.Loadings.addDeptLoading = false;

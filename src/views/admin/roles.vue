      ...
      <el-container>
          ...
          >
            ...
            <el-row v-for="(todo,index) of roleList" :key="index" class="role-list" align="center">
              <div @mouseenter="editShow=todo.id" @mouseleave="editShow=null">
                <el-row>
                  <el-col :span="18" >
                    <div style="cursor:pointer; width:100%" @click="getRoleUserList(todo.id)">{{todo.name}}</div>
                  </el-col>
        ...
        <el-main>
          <div class="t-header">
            <el-row>
              <el-col :span="6">
                拥有<span style="font-weight:700;color:#ed4014">{{name}}</span>角色的用户有：
              </el-col>
              <el-col :span="3">
                <el-button
                  type="primary"
        ...
        <el-dialog :visible.sync="add_role_show" title="添加角色" width="480px" top="5vh">
          ...
          >
            ...
            <el-form-item label="角色分类" prop="role_sort" v-if="is_superuser">
  ...
  methods: {
    ...
    addRole(Type) {
      if (Type === 1) {
        ...
        add_Role({...this.addDeptRole, communityuuid:this.communityuuid})
          .then(({ data }) => {
            this.add_role_show = false;
            if (data.status === 0) {
              this.$message.success(data.msg);
              //角色列表
              getRoles({communityuuid:this.communityuuid}).then(({ data }) => {
    ...
    getList() {
      ...
      getRoles({communityuuid:this.communityuuid}).then(({ data }) => {
        console.log(data)
        this.roleList = [...data.msg];
        this.manage_role = data.auth.manage_role
        this.is_superuser = data.superuser
      });
    },
    getRoleUserList(id) {
      console.log(id)
      this.roleid = id;
      const roleid = id;
      getUsersRole({ id: roleid, communityuuid:this.communityuuid }).then(({ data }) => {

  ...
  <div id="asset-list">
    ...
    <el-dialog title="注意事项" :visible.sync="dialogVisible" width="365px">
      <div style="padding-left:5px;padding-bottom:20px;padding-right:5px">
        ...
        <div style="padding-top:2px;font-size:12px">
          <h4>部门：</h4>
          <div style="padding-top:2px">若用户所属部门不存在，则返回“未能找到该对应部门”。</div>
        </div>
        <div style="padding-top:2px;font-size:12px">
          <h4>是否启用：</h4>
...
export default {
  ...
  methods: {
    ...
    returnAssemblingData(data) {
      ...
      uploadUsers({...data,communityuuid:localStorage.getItem("communityuuid")})
        .then(({ data }) => {
          if (data.status === 0) {
            this.$notify({
            ...
            this.$router.push({
              name: "profession",
              query:{
                communityuuid:this.$route.query.communityuuid
              }
            });
          } else {

<template>
  <div>
      ...
      <el-form inline>
        ...
        <el-form-item>
          <el-checkbox v-model="queryForm.recommend">推荐车务</el-checkbox>
        </el-form-item>
        <el-form-item>
          <el-button-group>
    ...
    <el-dialog class="dialog" width="700px"  lock-scroll center 
...
<script>
import { viewProducts, viewProductsClass, 
  updateProduct, deleteProduct } from "@/api/product";
...
export default {
  ...
  methods: {
    ...
    getCategories() {
      getCategory({ pc: "", categorytype:2 }).then(({ data }) => {
        if (data.status == 0) { 
    ...
    //删除
    delProduct(item) {
      this.$confirm("此操作将永久删除该车务，是否继续?", "提示", {
        confirmButtonText: "确认",
        cancelButtonText: "取消",
        type: "warning",
      }).then(() => {

    ...
    >
      ...
      <el-table-column label="活动图片" align="center">
        <template slot-scope="scope">
          <el-image
            :src="$store.state.BASE_URL_IMAGE+'/'+scope.row.image"
...
export default {
    ...
    methods: {
        //获取活动列表
        getActivity(){
            getActivityList({pc:"",communityuuid:this.communityuuid}).then(({data})=>{
                console.log(data)
                if(data.status == 0){
        ...
        //获取活动管理权限
        getAuth(){
            getActivityAuth({communityuuid:this.communityuuid}).then(({data}) =>{
              console.log(data)
              if(data.status == 0){
                this.auth = data.msg  
              }
        ...
        //关闭添加活动弹出框
        cancel() {
            this.actForm.status = 0; 
            this.postSpread()
        },
        postSpread(){
          addActivity(this.actForm).then(({data}) => {
                if(data.status == 0){
                    this.$message.success(data.msg);
        ...
        handleSuccess(response, file, fileList) {
          console.log(response)
            if (response.status == 0) {
                ...
                this.actForm["image"]= response.msg; 
            } else {
    ...
    created() {
      ...
      this.actForm['communityuuid'] = this.communityuuid
      this.getActivity(),
      this.getAuth() 
    }
};
